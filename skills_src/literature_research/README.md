# `skills_src/literature_research/` — AI 文献调研模块

本目录是 pySciWS 的**结构化文献知识库 + 检索/精读工具链**，由 AI Agent（Qoder）主导维护、用户审校。
它是 `literature-research` skill 的**代码与数据后端**，二者分工如下：

| 位置 | 角色 | 内容 |
|---|---|---|
| `skills_src/literature_research/`（本目录） | **代码 + 数据** | `tools/` 工具链、`papers/` 笔记、`shortlists/` 检索快照、`reviews/` 综述、`INDEX.md` 索引、`cache/` 缓存 |
| `.qoder/skills/literature-research/` | **Skill（说明书）** | `SKILL.md` + `references/`，教 AI 何时、如何调用本目录的 `research` CLI |

> **命名对应**：模块目录 `literature_research`（下划线，合法 Python 包名）↔ skill `literature-research`（连字符，skill 命名规范）。一一对应，见名知意。
> **`skills_src/`** 是项目预留的**模块技能代码根目录**，未来其它 skill 的后端代码也统一放这里。
...
| `skills_src/literature_research/` | **AI Agent** + 用户审校 | 单篇论文笔记、检索快照、综述草稿、工具脚本 | **是**（除 `cache/`、PDF） |
...
```
skills_src/literature_research/
├── README.md                # 本文件
...
│   ├── config.py            # 统一配置加载（读项目根 .env，暴露 settings）
...
```
.venv\Scripts\python.exe -m skills_src.literature_research.tools.research <子命令> …
```
...
# 或：uv run python -m skills_src.literature_research.tools.research doctor
```
