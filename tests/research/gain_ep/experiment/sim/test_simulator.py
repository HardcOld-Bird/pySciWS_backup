...
测试 pysci.research.gain_ep.experiment.sim.simulator 模块中 SimScanner 类及相关数据类/辅助函数。
...
from pysci.research.gain_ep.experiment.sim.simulator import (
    ScanResult,
    SimScanner,
    _format_complex,
    _parse_complex_str,
...
class TestScanResult:
    ...
    def _make_result(res: int = 3) -> ScanResult:
        ...
        return ScanResult(
            ...
            floquet_initial_r=np.random.randn(3) + 1j * np.random.randn(3),
            exp_eight_rho=np.zeros((res, res)),
            exp_eight_alpha=np.ones((res, res)),
            exp_eight_rho_relaxed=np.zeros((res, res)),
            exp_floquet_rho=np.zeros((res, res)),
            exp_floquet_alpha=np.ones((res, res)),
    ...
    def test_shapes(self):
        ...
        assert result.exp_eight_steady_l.shape == (res, res, 8)
        assert result.exp_eight_steady_r.shape == (res, res, 8)
        assert result.eight_initial_l.shape == (8,)
        assert result.eight_initial_r.shape == (8,)
...
class TestSaveScanResult:
    ...
    def test_save_creates_npz(self, tmp_path):
        ...
        result = ScanResult(
            ...
            exp_eight_steady_l=np.ones((res, res, 8), dtype=complex),
            exp_eight_steady_r=np.ones((res, res, 8), dtype=complex),
            exp_floquet_steady_l=np.ones((res, res, 8), dtype=complex),
            exp_floquet_steady_r=np.ones((res, res, 8), dtype=complex),
            ...
            floquet_initial_r=np.ones(3, dtype=complex) * 0.05,
            exp_eight_rho=np.zeros((res, res)),
            exp_eight_alpha=np.ones((res, res)),
            exp_eight_rho_relaxed=np.zeros((res, res)),
            exp_floquet_rho=np.zeros((res, res)),
    ...
    def test_save_load_roundtrip(self, tmp_path):
        ...
        result = ScanResult(
            ...
            exp_eight_steady_l=np.zeros((res, res, 8), dtype=complex),
            exp_eight_steady_r=np.zeros((res, res, 8), dtype=complex),
            exp_floquet_steady_l=np.zeros((res, res, 8), dtype=complex),
            exp_floquet_steady_r=np.zeros((res, res, 8), dtype=complex),
            eight_initial_l=np.zeros(8, dtype=complex),
            eight_initial_r=np.zeros(8, dtype=complex),
            floquet_initial_l=np.zeros(3, dtype=complex),
            floquet_initial_r=np.zeros(3, dtype=complex),
            exp_eight_rho=np.zeros((res, res)),
            exp_eight_alpha=np.ones((res, res)),
            exp_eight_rho_relaxed=np.zeros((res, res)),
            exp_floquet_rho=np.zeros((res, res)),
...
class TestPlotScanResults:
    ...
    def test_creates_four_plots(self, tmp_path):
        ...
        expected_files = [
            ...
            "exp_eight_response_ratio.png",
            "floquet_gain_point1.png",
            "floquet_target_point1.png",
            "exp_floquet_response_ratio.png",
        ]
