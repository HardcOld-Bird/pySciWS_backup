...
- 代码组织为单一 **src-layout 安装包 `pysci`**（通过 uv 以 editable 方式装入唯一 `.venv`），按“共享工具层 / 研究层 / skill 后端”分层；非代码资产（.mph、storage、笔记、参考资料）放在平行资产树 `data/`。
...
├── data/                       # 平行资产树——数据树（与 src/pysci/ 镜像，非代码资产，不入包）
│   ├── research/<n>_<name>/    #   ⟷ src/pysci/research/<name>/（当前仅 1_gain_ep）
│   │   ├── theory/             #     理论笔记 / PDF
...
- **data/research/<n>_<name>/storage**: 按研究线组织，存储数据文件和图表（可再生，不入版本控制）
- **包名与资产目录**: Python 包路径不能以数字开头，故资产目录 `data/research/1_gain_ep` 对应包路径 `pysci.research.gain_ep`（数字序号只保留在资产树目录名中）
...
# gain_ep 研究资产根（= data/research/1_gain_ep）
...
**标准流程**：
1. 在 COMSOL GUI 中创建和调试模型，保存到 `data/research/<n>_<name>/experiment/mphs/`（包含多个版本，除mph外均可作为文本文件读取，方便你查看仿真文件细节）
...
- foundations 各主题存储在根 `参考资料（手动）/<主题>/`；研究线专属资料存储在 `data/research/<n>_<name>/参考资料/`
...
- **存储规范**：所有输出文件必须保存到对应研究线的 `data/research/<n>_<name>/storage/` 子目录
