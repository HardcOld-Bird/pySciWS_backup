...
模块路径：`pysci.research.gain_ep.experiment.sim.simulator`
...
class ScanResult:
    ...
    Attributes:
        ...
        exp_eight_steady_l: 真实实验系统稳态（8 周期增益，左入射），shape (res, res, 8)
        exp_eight_steady_r: 真实实验系统稳态（8 周期增益，右入射），shape (res, res, 8)
        exp_floquet_steady_l: 真实实验系统稳态（Floquet 增益，左入射），shape (res, res, 8)
        exp_floquet_steady_r: 真实实验系统稳态（Floquet 增益，右入射），shape (res, res, 8)
        eight_initial_l: 8 周期模式左入射初态，shape (8,)
        eight_initial_r: 8 周期模式右入射初态，shape (8,)
        floquet_initial_l: Floquet 模式左入射初态，shape (3,)
        floquet_initial_r: Floquet 模式右入射初态，shape (3,)
    ...
    # 真实实验系统稳态（双方向入射）
    exp_eight_steady_l: np.ndarray  # shape (res, res, 8), complex128, 左入射
    exp_eight_steady_r: np.ndarray  # shape (res, res, 8), complex128, 右入射
    exp_floquet_steady_l: np.ndarray  # shape (res, res, 8), complex128, 左入射
    exp_floquet_steady_r: np.ndarray  # shape (res, res, 8), complex128, 右入射

    # 双方向初态
    eight_initial_l: np.ndarray  # shape (8,), complex128
    eight_initial_r: np.ndarray  # shape (8,), complex128
    floquet_initial_l: np.ndarray  # shape (3,), complex128
    floquet_initial_r: np.ndarray  # shape (3,), complex128
...
class _SimCache:
    ...
    封装 run_scan 中仅依赖于 f 的计算结果，
    避免 f 不变时重复执行耗时的 COMSOL 仿真。

    缓存范围（对应 run_scan 中的步骤）：
        - 步骤 2: 8 周期模式空管槽初态（左入射 + 右入射）
        - 步骤 3: 8 周期模式 8×8 传递矩阵
        - 步骤 7: Floquet 模式空管槽初态（左入射 + 右入射）
        - 步骤 8: Floquet 模式传递函数

    Attributes:
        f: 仿真频率 (Hz)
        eight_initial_l: 8 周期模式左入射初态，shape (8,), complex128
        eight_initial_r: 8 周期模式右入射初态，shape (8,), complex128
        eight_tf_matrix: 8 周期模式传递矩阵，shape (8, 8), complex128
        floquet_initial_l: Floquet 模式左入射初态，shape (3,), complex128
        floquet_initial_r: Floquet 模式右入射初态，shape (3,), complex128
        floquet_tf: Floquet 模式传递函数，shape (3,), complex128
    """

    f: float
    eight_initial_l: np.ndarray
    eight_initial_r: np.ndarray
    eight_tf_matrix: np.ndarray
    floquet_initial_l: np.ndarray
    floquet_initial_r: np.ndarray
    floquet_tf: np.ndarray

    def matches(self, f: float) -> bool:
        """检查缓存参数是否与给定频率匹配
...
class SimScanner:
    ...
    步骤 2/3/7/8 的结果仅依赖于 f 参数，
    当 f 不变时自动复用 ``storage/sim/sim_cache`` 中的缓存。
    ...
    #: 静态激励 AO 通道（左入射）
    _STATIC_AO_CHANNEL_L: str = "PXI1Slot2/ao0"

    #: 静态激励 AO 通道（右入射）
    _STATIC_AO_CHANNEL_R: str = "PXI1Slot2/ao1"
    ...
    def _load_sim_cache(self, f: float) -> _SimCache | None:
        """尝试加载仿真缓存
    ...
    def run_scan(
        ...
        Args:
            f: 仿真频率 (Hz)，默认 3430
            cr: 管槽声速实部放缩因子（扫描中心值），默认 1.006
            cr_min: cr 扫描下限，默认 1.004
            cr_max: cr 扫描上限，默认 1.008
            ci: 管槽声速虚部放缩因子（扫描中心值），默认 -0.073
            ci_min: ci 扫描下限，默认 -0.075
            ci_max: ci 扫描上限，默认 -0.071
            res: cr/ci 共用扫描分辨率（每轴取值数），默认 10
            swap_static_ao: 是否对调左/右入射静态 AO 通道，默认 False。
                为 True 时，原本的左入射通道变为右入射，右入射变为左入射。
        ...
        exp_initial_state_l: np.ndarray | None = None
        exp_initial_state_r: np.ndarray | None = None
        if tf_data is not None:
            ...
            exp_initial_state_r = np.asarray(
                tf_df.loc[ch_r, list(self._POINT_CHANNELS)].values,
                dtype=np.complex128,
            )
            logger.info(
                f"已加载实验传递矩阵: shape={exp_tf_matrix.shape}, "
                f"左初态模长={np.abs(exp_initial_state_l)}, "
                f"右初态模长={np.abs(exp_initial_state_r)}"
            )
        else:
        ...
        # ---- 加载仿真缓存 ----
        # 步骤 2/3/7/8 的结果仅依赖于 f，
        # 当 f 不变时可复用缓存，避免重复执行耗时的 COMSOL 仿真。
        cache = self._load_sim_cache(f)
        ...
        # 2. 空管槽 (cr=1, ci=0) → 双方向初态（可缓存）
        if cache_hit:
            logger.info("2/10: eight_probes_single 空管槽双方向初态 [缓存]...")
            eight_initial_l = cache.eight_initial_l
            eight_initial_r = cache.eight_initial_r
        else:
        ...
        logger.info(f"  右初态: {np.abs(eight_initial_r)}")
        ...
        eight_gains = self._solve_gains_batch(
            eight_target, eight_initial_l, eight_tf_matrix
        )
        logger.info(
        ...
        # 5. 使用仿真增益 + 实验传递矩阵计算真实实验系统稳态（双方向入射）
        exp_eight_steady_l = np.zeros((res, res, 8), dtype=np.complex128)
        exp_eight_steady_r = np.zeros((res, res, 8), dtype=np.complex128)
        if exp_tf_matrix is not None and exp_initial_state_l is not None:
            ...
            logger.info(
                f"  右入射稳态: mean |steady|={np.mean(np.abs(exp_eight_steady_r)):.4f}"
            )
        ...
        # 8. 空管槽 → 双方向初态（可缓存）
        if cache_hit:
            logger.info("7/10: floquet_probes_single 空管槽双方向初态 [缓存]...")
            floquet_initial_l = cache.floquet_initial_l
            floquet_initial_r = cache.floquet_initial_r
        else:
        ...
        logger.info(f"  右初态: {np.abs(floquet_initial_r)}")
        ...
        initial_point1 = floquet_initial_l[2]  # point1 初态（左入射）
        ...
        for probe_idx in range(3):
            ...
            initial_probe = floquet_initial_l[probe_idx]  # scalar
        ...
        # 11. 使用 Floquet 增益 + 实验传递矩阵计算真实实验系统稳态（双方向入射）
        exp_floquet_steady_l = np.zeros((res, res, 8), dtype=np.complex128)
        exp_floquet_steady_r = np.zeros((res, res, 8), dtype=np.complex128)
        if exp_tf_matrix is not None and exp_initial_state_l is not None:
            ...
            logger.info(
                f"  右入射稳态: mean |steady|={np.mean(np.abs(exp_floquet_steady_r)):.4f}"
            )
        ...
        result = ScanResult(
            ...
            exp_eight_steady_l=exp_eight_steady_l,
            exp_eight_steady_r=exp_eight_steady_r,
            exp_floquet_steady_l=exp_floquet_steady_l,
            exp_floquet_steady_r=exp_floquet_steady_r,
    ...
    def _save_scan_result(
        ...
        np.savez_compressed(
            ...
            eight_initial_r=result.eight_initial_r,
            eight_tf_matrix=result.eight_tf_matrix,
            eight_gains=result.eight_gains,
            # Floquet 模式
            floquet_target=result.floquet_target,
            floquet_initial_l=result.floquet_initial_l,
            floquet_initial_r=result.floquet_initial_r,
            floquet_tf=result.floquet_tf,
            floquet_gains=result.floquet_gains,
            # 真实实验系统稳态（双方向）
            exp_eight_steady_l=result.exp_eight_steady_l,
            exp_eight_steady_r=result.exp_eight_steady_r,
            exp_floquet_steady_l=result.exp_floquet_steady_l,
            exp_floquet_steady_r=result.exp_floquet_steady_r,
            # 收敛性分析
    ...
    def _plot_scan_results(self, result: ScanResult, save_dir: Path) -> None:
        ...
        # ------------------------------------------------------------------
        # 第 3 幅: 真实实验系统左/右入射响应比（8 周期增益）
        # ------------------------------------------------------------------
        # 响应比 = 左入射8通道幅值之和 / 右入射8通道幅值之和
        sum_eight_l = np.sum(np.abs(result.exp_eight_steady_l), axis=2)
        ...
        if result.exp_eight_rho is not None:
            conv_eight = {
                ...
                "cr_values": cr,
                "ci_values": ci,
                "data_for_best": response_ratio_eight,
        ...
        # ------------------------------------------------------------------
        # 第 6 幅: 真实实验系统左/右入射响应比（Floquet 增益）
        # ------------------------------------------------------------------
        sum_floquet_l = np.sum(np.abs(result.exp_floquet_steady_l), axis=2)
        ...
        if result.exp_floquet_rho is not None:
            conv_floquet = {
                ...
                "cr_values": cr,
                "ci_values": ci,
                "data_for_best": response_ratio_floquet,
    ...
    def _plot_discrete_heatmap(
        ...
        Args:
            ...
            convergence_overlay: 收敛性叠加数据字典，包含:
                ...
                - best_metric_label: 最优值指标名称（可选，默认"稳态模长均值"）
        ...
        if convergence_overlay is not None:
            ...
            # ---- 标记安全区中指标最大的参数点 ----
            if np.any(safe_mask):
                masked_data = np.where(safe_mask, data_for_best, -np.inf)
