...
模块路径：`pysci.research.gain_ep.experiment.analyze.plot`
...
def _render_subplot(
    ...
    # normal / partial 均绘制 picked_area 轮廓
    cx, cy = picked_center.x, picked_center.y
    r = picked_area_radius
...
def prepare_comprehensive_experiment_data(
    # --- 10 组数据路径 ---
    left_r_0_passive_folder: str | Path | None = None,
    left_r_0_active_folder: str | Path | None = None,
    left_r_minus1_passive_folder: str | Path | None = None,
    left_r_minus1_active_folder: str | Path | None = None,
    right_r_plus1_passive_folder: str | Path | None = None,
    right_r_plus1_active_folder: str | Path | None = None,
    right_r_0_passive_folder: str | Path | None = None,
    right_r_0_active_folder: str | Path | None = None,
    ...
    Args:
        left_r_0_passive_folder: 左侧 r=0 无源数据文件夹路径
        left_r_0_active_folder: 左侧 r=0 有源数据文件夹路径
        left_r_minus1_passive_folder: 左侧 r=-1 无源数据文件夹路径
        left_r_minus1_active_folder: 左侧 r=-1 有源数据文件夹路径
        right_r_plus1_passive_folder: 右侧 r=+1 无源数据文件夹路径
        right_r_plus1_active_folder: 右侧 r=+1 有源数据文件夹路径
        right_r_0_passive_folder: 右侧 r=0 无源数据文件夹路径
        right_r_0_active_folder: 右侧 r=0 有源数据文件夹路径
    ...
    folder_map = {
        "l_r0_p": left_r_0_passive_folder,
        "l_r0_a": left_r_0_active_folder,
        "l_rm1_p": left_r_minus1_passive_folder,
        "l_rm1_a": left_r_minus1_active_folder,
        "r_rp1_p": right_r_plus1_passive_folder,
        "r_rp1_a": right_r_plus1_active_folder,
        "r_r0_p": right_r_0_passive_folder,
        "r_r0_a": right_r_0_active_folder,
...
def plot_comprehensive_experiment(
    ...
    Args:
        ...
        k_angle_deg: 入射波传播方向角度 (度)，0=x正向，90=y正向，默认 0。
            fourier 模式下：中央四幅散射场子图统一使用此角度，
            背景场使用 k_angle_deg+180（与入射相反的方向）。
    ...
    # ==================================================================
    # passive = passive_raw - background
    p_l_r0 = combine_point_tf_data_list(tf["l_r0_p"], tf["r_bg"], mode="minus")
    p_l_rm1 = combine_point_tf_data_list(tf["l_rm1_p"], tf["l_bg"], mode="minus")
    p_r_rp1 = combine_point_tf_data_list(tf["r_rp1_p"], tf["l_bg"], mode="minus")
    p_r_r0 = combine_point_tf_data_list(tf["r_r0_p"], tf["r_bg"], mode="minus")

    # active = active_raw - background
    a_l_r0 = combine_point_tf_data_list(tf["l_r0_a"], tf["r_bg"], mode="minus")
    a_l_rm1 = combine_point_tf_data_list(tf["l_rm1_a"], tf["l_bg"], mode="minus")
    a_r_rp1 = combine_point_tf_data_list(tf["r_rp1_a"], tf["l_bg"], mode="minus")
    a_r_r0 = combine_point_tf_data_list(tf["r_r0_a"], tf["r_bg"], mode="minus")
    ...
    # 4. 确定每个子图的状态
    # ==================================================================
    def _get_state(main_key: str, bg_key: str) -> str:
        if tf[main_key] is None:
            return "unavailable"
    ...
    passive_info = [
        (p_l_r0, _get_state("l_r0_p", "r_bg"), r"$r_0$ (left)"),
        (p_r_rp1, _get_state("r_rp1_p", "l_bg"), r"$r_{+1}$ (right)"),
        (p_l_rm1, _get_state("l_rm1_p", "l_bg"), r"$r_{-1}$ (left)"),
        (p_r_r0, _get_state("r_r0_p", "r_bg"), r"$r_0$ (right)"),
    ]
    active_info = [
        (a_l_r0, _get_state("l_r0_a", "r_bg"), r"$r_0$ (left)"),
        (a_r_rp1, _get_state("r_rp1_a", "l_bg"), r"$r_{+1}$ (right)"),
        (a_l_rm1, _get_state("l_rm1_a", "l_bg"), r"$r_{-1}$ (left)"),
        (a_r_r0, _get_state("r_r0_a", "r_bg"), r"$r_0$ (right)"),
    ]
    bg_state = "normal" if bg_data is not None else "unavailable"

    # ==================================================================
    # 5. 区域选取 + 振幅积分
    #    k_angle_deg 表示入射波传播方向。
    #    fourier 积分使用 exp(+ik·r)，当 k_angle 与波传播方向一致时结果最大。
    #    - 中央四幅散射场子图 → 统一使用 k_angle_deg
    #    - 背景场（入射波，与散射场传播方向相反）→ 使用 k_angle_deg + 180
    # ==================================================================
    ...
    k_reflect = k_angle_deg + 180.0  # 反射方向（与入射相反），仅用于背景场
    ...
    # 提取振幅和复数积分值
    p_amps = [r[0] for r in p_results]
    a_amps = [r[0] for r in a_results]
    bg_amp_val, bg_complex_val = bg_result
    p_complex = [r[1] for r in p_results] if integral_mode == "fourier" else None
    a_complex = [r[1] for r in a_results] if integral_mode == "fourier" else None
    ...
    # S矩阵分母：背景场的积分幅值（abs模式为平均|H|，fourier模式为|傅里叶积分|）
    bg_amp_abs = abs(bg_amp_val) if bg_amp_val else 1.0

    # ColorBar 标签归一化因子：背景场的最大瞬时场值 max(|Re(bg)|)。
    # 注意：子图着色数据为瞬时场值 Re(complex_amplitude)，因此归一化因子
    # 必须使用同类量（瞬时场极值），而非傅里叶积分幅值（后者包含 k/(2π) 前缀因子，
    # 量级远大于瞬时场值，会导致 ColorBar 标签被错误地缩小）。
    if bg_data is not None:
        _bg_inst = np.array([p["complex_amplitude"].real for p in bg_data])
        bg_inst_max = float(np.max(np.abs(_bg_inst))) if _bg_inst.size else 0.0
    else:
        bg_inst_max = 0.0
    cbar_norm_factor = bg_inst_max if bg_inst_max > 0.0 else 1.0

    # fourier 模式: 中央四幅子图统一使用 k_angle_deg；背景子图使用 k_angle_deg+180
    center_k_angles = [k_angle_deg, k_angle_deg, k_angle_deg, k_angle_deg]
    ...
    def _build_figure(
        ...
        vmin: float,
        vmax: float,
        integral_results: list[tuple[float, complex | None]],
        bg_integral: tuple[float, complex | None],
    ) -> Figure:
        ...
        # fourier 模式: 在背景子图和中央四幅子图上绘制波矢箭头 + 积分值文字
        if integral_mode == "fourier":
            half_lambda_mm = (np.pi / k_modulus) * 1000.0  # 半波长 (mm)

            # 背景子图 + 中央四幅子图的波矢箭头
            # 背景使用 k_angle_deg+180（反射方向）；中央四幅统一使用 k_angle_deg
            arrow_targets = [(ax_bg, k_reflect, bg_st)]
            arrow_targets += [
                (ax, ka, state)
                for (ax, ka), (_, state, _) in zip(
                    zip(center_axes, center_k_angles), info_list
                )
            ]
            for ax_arrow, k_angle, state in arrow_targets:
                # 跳过不可用状态的子图，避免箭头异常
                if state == "unavailable":
                    continue
                k_rad = math.radians(k_angle)
                dx_mm = half_lambda_mm * math.cos(k_rad)
                dy_mm = half_lambda_mm * math.sin(k_rad)
                # 以数据坐标系右下角为起点
                x_lo, x_hi = ax_arrow.get_xlim()
                y_lo, y_hi = ax_arrow.get_ylim()
                x_range = x_hi - x_lo
                y_range = y_hi - y_lo
                # 避免除零（数据为空时 range 可能为 0）
                if x_range == 0 or y_range == 0:
                    continue
                start_x = x_lo + 0.85 * x_range
                start_y = y_lo + 0.08 * y_range
                # 限制箭头长度不超过子图边长的 25%，防止溢出
                max_dx = 0.25 * x_range
                max_dy = 0.25 * y_range
                if abs(dx_mm) > max_dx and dx_mm != 0:
                    scale = max_dx / abs(dx_mm)
                    dx_mm *= scale
                    dy_mm *= scale
                if abs(dy_mm) > max_dy and dy_mm != 0:
                    scale = max_dy / abs(dy_mm)
                    dx_mm *= scale
                    dy_mm *= scale
                ax_arrow.annotate(
                    "",
            ...
            # 在每幅子图下方标注积分幅值结果（置于 x 轴标题下方）
            for idx, ax_text in enumerate(center_axes):
                amp_val, cplx_val = integral_results[idx]
                bbox = ax_text.get_position()
                text_str = f"|∫| = {amp_val:.4f}"
            ...
            bg_amp_result, bg_cplx_result = bg_integral
            bg_bbox = ax_bg.get_position()
            bg_text_str = f"|∫| = {bg_amp_result:.4f}"
        ...
        cbar.set_ticklabels([f"{t / cbar_norm_factor:.2f}" for t in ticks])
    ...
    def _build_s_matrix(
        ...
        if (
            ...
            s_c = [
                [_safe(cplx[0]) / bg_cplx, _safe(cplx[1]) / bg_cplx],
                [_safe(cplx[2]) / bg_cplx, _safe(cplx[3]) / bg_cplx],
            ]
    ...
    # 9. 绘制 Passive 图
    # ==================================================================
    # 构建图标题（fourier 模式下记录 k_angle_deg 参数）
