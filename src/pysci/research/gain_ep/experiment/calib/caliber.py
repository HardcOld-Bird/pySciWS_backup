...
模块路径：`pysci.research.gain_ep.experiment.calib.clb`
...
class CaliberAnemone:
    ...
    ## 使用示例：
    ```python
    from pysci.research.gain_ep.experiment.use.clb import CaliberAnemone
...
class CaliberOctopus:
    ...
    ## 使用示例：
    ```python
    from pysci.research.gain_ep.experiment.use.clb import CaliberOctopus
    from pysci.research.gain_ep.experiment.analyze import init_sampling_info
...
class CaliberFishNet(CaliberOctopus):
    ...
    ## 使用示例：
    ```python
    from pysci.research.gain_ep.experiment.use.clb import CaliberFishNet
    from pysci.research.gain_ep.experiment.analyze import init_sampling_info
    ...
    def calibrate(
        ...
        Args:
            starts_num: 独立校准的次数，默认为3。
                这是提高校准精度的主要手段，建议设置较高的值。
            chunks_per_start: 每次启动采集的连续chunk数，默认为3
            apply_filter: 是否应用滤波，默认为True
            result_folder: 可选，结果保存文件夹路径。如果为None，将使用默认路径
...
class FrequencyOptimizer:
    ...
    ## 双声源通道支持：
        实验平台具有两个声源通道（左入射和右入射），分别由 ao_channel_max 和
        ao_channel_min 控制。两种入射角度对传声器阵列产生的响应差距很大，
        一侧响应较大，另一侧响应较小。

    ## 优化模式：
        - **amplitude mode**: 在每个频率点先后测量两个AO通道，计算二者幅值之和
          的比值（metric_max/metric_min），寻找使该比值最大的频率。
          使用区间收缩法，仅假设目标函数是单峰的。
        - **phase mode**: 仅使用 ao_channel_max，寻找使相邻通道相位差与π的
          偏差为零的频率。使用线性插值（割线法）迭代逼近零点。
    ...
    ## 使用示例：
    ```python
    from pysci.research.gain_ep.experiment.calib import FrequencyOptimizer
    ...
    optimizer = FrequencyOptimizer(
        ...
        ao_channel_max="PXI1Slot2/ao0",  # 主声源（响应较大）
        ao_channel_min="PXI1Slot2/ao1",  # 辅声源（响应较小）
        amplitude=0.01,
    )

    # 执行频率优化（默认 amplitude 模式）
    ...
    def __init__(
        ...
        Args:
            ai_channels: AI 通道名称元组（超表面管槽中的传声器）。
                         典型为8个通道，但允许多于或少于8个。
            ao_channel_max: AO 通道名称（主声源通道，预期产生较大响应）。
            ao_channel_min: AO 通道名称（辅声源通道，预期产生较小响应）。
        ...
        self._ai_channels = ai_channels
        self._ao_channel_max = ao_channel_max
        self._ao_channel_min = ao_channel_min
        self._amplitude = amplitude
        ...
        logger.info(
            ...
            f"AI通道数: {len(ai_channels)}, "
            f"AO通道(max): {ao_channel_max}, "
            f"AO通道(min): {ao_channel_min}, "
            f"幅值: {amplitude}V"
        )
    ...
    def _compute_metric(
        ...
        注意：此方法仅计算单个AO通道的指标。amplitude模式下的双通道
        比值（metric_max/metric_min）由optimize方法在调用本方法后计算。
    ...
    def optimize(
        ...
        - **amplitude mode**: 在每个频率点先后测量两个AO通道（ao_channel_max和
          ao_channel_min），计算二者幅值之和的比值（metric_max/metric_min），
          寻找使该比值最大的频率。仅假设目标函数是单峰的（先增后减），
          使用区间收缩法逐步逼近极大值。适用于双入射角实验平台。
        - **phase mode**: 寻找使相邻通道相位差与π的偏差为零的频率。
          仅使用ao_channel_max，通过线性插值（割线法）迭代逼近零点。
          适用于理想化的无限周期模型。

        Args:
            mode: 优化模式，"amplitude" 或 "phase"，默认 "amplitude"
            initial_freq: 初始测量频率（Hz），默认3430.0
            second_freq: 第二个测量频率（Hz），默认3420.0
            max_iterations: 最大迭代次数，默认10
            tolerance: 收敛容差（归一化比例），默认0.001。
                      所有metric均以第一次测量值的绝对值为基准进行归一化，
                      因此tolerance表示"相对于初始值的比例"。
                      - amplitude mode: 收敛判据为存在3个相邻测量点(p1,p2,p3)，
                        其中p2的ratio大于p1和p3，且两侧归一化ratio之差均 < tolerance
    ...
    def _measure_and_compute_metric(
        self,
        frequency: float,
        ...
        对于amplitude模式，先后测量ao_channel_max和ao_channel_min两个通道，
        计算二者的幅值之和比值（metric_max/metric_min）作为优化指标。
        对于phase模式，仅测量ao_channel_max并计算相位偏差。

        Args:
            frequency: 测量频率（Hz）
            mode: 优化模式
            measurement_index: 当前测量序号
            starts_num: 独立校准次数
            chunks_per_start: 每次采集的chunk数
            settle_time: 稳定等待时间
            result_path: 结果保存路径

        Returns:
            tuple of:
            - metric (float): 指标值（amplitude模式为ratio，phase模式为相位偏差）
            - amplitudes (ndarray): 主通道的各通道幅值数组
            - phases (ndarray): 主通道的各通道相位数组
        """
        if mode == "amplitude":
            # 测量 ao_channel_max
            tf_data_max = self._measure_at_frequency(
                frequency,
                ao_channel=self._ao_channel_max,
            ...
            tf_data_min = self._measure_at_frequency(
                frequency,
                ao_channel=self._ao_channel_min,
                measurement_index=measurement_index,
                starts_num=starts_num,
                chunks_per_start=chunks_per_start,
                settle_time=settle_time,
            ...
            logger.info(
                f"频率 {frequency:.2f}Hz (amplitude mode): "
                f"metric_max={metric_max:.6f}, metric_min={metric_min:.6f}, "
                f"ratio={ratio:.6f}"
            )

            return ratio, amps_max, phases_max
        else:
            # Phase mode: 仅测量 ao_channel_max
            tf_data = self._measure_at_frequency(
                frequency,
                ao_channel=self._ao_channel_max,
                measurement_index=measurement_index,
                starts_num=starts_num,
    ...
    def _iterate_phase(
        ...
        """Phase mode 迭代：割线法逼近相位偏差零点（仅使用ao_channel_max）"""
        ...
        for iteration in range(max_iterations):
            ...
            # 在预测频率处测量（使用 _measure_and_compute_metric）
            assert isinstance(predicted_freq, float)
            measurement_counter += 1
            metric_new, amps_new, phases_new = self._measure_and_compute_metric(
                predicted_freq,
                mode="phase",
    ...
    def _iterate_amplitude(
        ...
        """
        Amplitude mode 迭代：区间收缩法寻找幅值比（ratio）的极大值

        目标函数为 metric_max/metric_min（两个AO通道的幅值之和比值），
        仅假设目标函数是单峰的（先增后减），通过逐步缩减区间逼近极大值。
        ...
        2. 每次迭代：
           ...
           c. 找到当前ratio最大的点（peak candidate）
           d. 若极值点在边缘：向该方向延伸一步
        ...
        measurement_counter += 1
        metric_3, amps_3, phases_3 = self._measure_and_compute_metric(
            third_freq,
            mode="amplitude",
            measurement_index=measurement_counter,
            starts_num=starts_num,
        ...
        for iteration in range(max_iterations):
            ...
            # 找到当前ratio最大的点的索引（在sorted_hist中）
            best_idx = max(range(n_points), key=lambda i: sorted_hist[i]["metric"])
            ...
            # 在新频率处测量（使用 _measure_and_compute_metric）
            measurement_counter += 1
            metric_new, amps_new, phases_new = self._measure_and_compute_metric(
                new_freq,
                mode="amplitude",
        ...
        logger.warning(
            f"达到最大迭代次数 {max_iterations}，"
            f"使用幅值比最大的频率: {best_freq:.2f}Hz "
            f"(ratio={best_entry['metric']:.6f})"
        )
        self._finalize(best_freq, resolved_result_path)
        return best_freq
    ...
    def plot_result(self, save_path: str | Path | None = None) -> None:
        ...
        - **amplitude mode**: 横轴频率，纵轴幅值比（metric_max/metric_min），测量点连线+峰值标记
        ...
        if mode == "phase":
            ...
            mode_label = "Amplitude"
        ax.set_title(
            f"频率优化结果 ({mode_label} mode)\n"
            f"AI通道数: {len(self._ai_channels)}, "
            f"AO通道(max): {self._ao_channel_max}, "
            f"AO通道(min): {self._ao_channel_min}",
            fontsize=14,
            pad=15,
        )
        ax.grid(True, alpha=0.3, linestyle="--", linewidth=0.8)
    ...
    def _plot_trend(
        ...
        if mode == "phase":
            ...
            ax.scatter(
                ...
                linewidths=1.5,
                zorder=6,
                label=f"最大幅值比点: {frequencies[best_idx]:.2f}Hz",
            )
...
class PowerTester:
    ...
    ## 使用示例：
    ```python
    from pysci.research.gain_ep.experiment.use.clb import PowerTester
