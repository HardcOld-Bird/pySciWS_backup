"""
# pysci.research.gain_ep.experiment 日志管理系统

模块路径：`pysci.research.gain_ep.experiment.logger`
...
## 使用示例:
    ### 在模块中使用：
    ```python
    from pysci.research.gain_ep.experiment.logger import get_logger
    ...
    ### 设置简要模式：
    ```python
    from pysci.research.gain_ep.experiment.logger import set_brief_mode
...
class SmartLevelFilter(logging.Filter):
    ...
    def _is_top_level_module(self, logger_name: str) -> bool:
        ...
        顶层模块的判断逻辑：
        1. 检查是否为pysci.research.gain_ep.experiment的直接子包（如pysci.research.gain_ep.experiment.use, pysci.research.gain_ep.experiment.move等）
        ...
        # 如果不是pysci.research.gain_ep.experiment包的模块，总是显示
        if not logger_name.startswith("pysci.research.gain_ep.experiment."):
            return True
...
class SweeperLoggerManager:
    """pysci.research.gain_ep.experiment 日志管理器"""
