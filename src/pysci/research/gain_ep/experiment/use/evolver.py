...
模块路径：`pysci.research.gain_ep.experiment.use.evolver`
...
class Evolver:
    ...
    ```python
    import numpy as np
    from pysci.research.gain_ep.experiment.analyze import init_sampling_info, get_sine
    from pysci.research.gain_ep.experiment.use import Evolver, load_evolved_waveform
    ...
    def simulate(
        ...
        cr: float,
        ci: float,
        mode: Literal["eight_probes", "floquet_probes"] = "floquet_probes",
        pick_max: bool = False,
        ...
        Args:
            ...
            mode: 增益系数选取模式。
                - ``"eight_probes"``：使用 8 周期模式的 8 个增益系数。
                - ``"floquet_probes"``（默认）：使用 Floquet 模式基于传声器 (point1)
                  的增益系数，将其复制到全部 8 个通道。
...
def load_evolved_waveform(
    ...
    rename_channel_dict: dict[str, str] | None = None,
) -> Waveform:
    """
    从文件加载演化最终波形。
    ...
    Args:
        ...
        amp_multiplier: 幅值缩放倍数，默认 1.0（不缩放）。加载后将所有通道的
            ...
            设为 2.0 可将幅值加倍。
        rename_channel_dict: 通道重命名映射字典，默认 None（不重命名）。
            键为原始通道名，值为目标通道名。用于实验中需要将某个 evolved_waveform
            的特定通道信号路由到其他通道的场景。所有键必须存在于原始波形的通道名中，
            重命名完成后若产生重复通道名则抛出 ValueError。
    ...
    Raises:
        ...
        ValueError: 当加载的数据不是有效的 Waveform，或 segments 不能整除采样点数时，
            ...
            或 rename_channel_dict 的键不全存在于波形通道中时，
            或重命名后产生重复通道名时。
    """
    ...
    if rename_channel_dict is not None:
        original_names = list(loaded_data.channel_names)
        # 检查所有键是否都存在于原始通道名中
        missing_keys = [k for k in rename_channel_dict if k not in original_names]
        if missing_keys:
            raise ValueError(
                f"rename_channel_dict 中的键不全存在于波形通道中，"
                f"以下通道名不存在: {missing_keys}。"
                f"原始通道名: {original_names}"
            )
        # 执行重命名：遍历原始通道名，若在字典中则替换
        new_names = tuple(
            rename_channel_dict.get(name, name) for name in original_names
        )
        # 检查重命名后是否有重复通道名
        if len(set(new_names)) != len(new_names):
            ...
            raise ValueError(
                f"rename_channel_dict 重命名后产生了重复通道名: {duplicates}。"
                f"重命名后通道名: {new_names}"
            )
        loaded_data.channel_names = new_names
        f_logger.info(
            f"已重命名通道: rename_channel_dict={rename_channel_dict}, "
            f"重命名后 channels={new_names}"
        )
