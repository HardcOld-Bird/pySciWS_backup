...
for i in range(8):
    print(
        f"  对{i + 1}: 幅值比 = {amp_ratio_1[i]:.4f}, 相位差 = {np.degrees(phase_diff_1[i]):+.2f}°"
    )

print("\n第一组统计:")
print(f"  幅值比均值: {np.mean(amp_ratio_1):.4f}, 标准差: {np.std(amp_ratio_1):.4f}")
print(
    f"  相位差均值: {np.degrees(np.mean(phase_diff_1)):+.2f}°, 标准差: {np.degrees(np.std(phase_diff_1)):.2f}°"
)

print("\n第二组:")
for i in range(8):
    print(
        f"  对{i + 1}: 幅值比 = {amp_ratio_2[i]:.4f}, 相位差 = {np.degrees(phase_diff_2[i]):+.2f}°"
    )

print("\n第二组统计:")
print(f"  幅值比均值: {np.mean(amp_ratio_2):.4f}, 标准差: {np.std(amp_ratio_2):.4f}")
print(
    f"  相位差均值: {np.degrees(np.mean(phase_diff_2)):+.2f}°, 标准差: {np.degrees(np.std(phase_diff_2)):.2f}°"
)
...
ax.axhline(
    ...
    color="red",
    linestyle="--",
    label=f"均值={np.mean(amp_ratio_1):.2f}",
)
...
ax.axhline(
    np.degrees(np.mean(phase_diff_1)),
    color="red",
    linestyle="--",
...
for i, (r, im) in enumerate(zip(H_1.real, H_1.imag)):
    ax.annotate(
        f"对{i + 1}", (r, im), textcoords="offset points", xytext=(5, 5), fontsize=8
    )
...
ax.axhline(
    ...
    color="red",
    linestyle="--",
    label=f"均值={np.mean(amp_ratio_2):.2f}",
)
...
ax.axhline(
    np.degrees(np.mean(phase_diff_2)),
    color="red",
    linestyle="--",
...
print(
    f"  幅值比均值: {np.mean(diff_amp_ratio_1):.4f}, 标准差: {np.std(diff_amp_ratio_1):.4f}"
)
print(
    f"  相位差均值: {np.degrees(np.mean(diff_phase_1)):+.2f}°, 标准差: {np.degrees(np.std(diff_phase_1)):.2f}°"
)
...
print(
    f"  幅值比均值: {np.mean(diff_amp_ratio_2):.4f}, 标准差: {np.std(diff_amp_ratio_2):.4f}"
)
print(
    f"  相位差均值: {np.degrees(np.mean(diff_phase_2)):+.2f}°, 标准差: {np.degrees(np.std(diff_phase_2)):.2f}°"
)
...
ax.axhline(
    np.degrees(np.mean(diff_phase_1)),
    color="red",
    linestyle="--",
...
ax.axhline(
    np.degrees(np.mean(diff_phase_2)),
    color="red",
    linestyle="--",
